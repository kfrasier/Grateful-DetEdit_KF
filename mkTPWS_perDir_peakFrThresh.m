...
% Setup variables:
baseDir = 'G:\HAT_A_06\HAT_A_06_d1-3\Dolphins'; % directory containing de_detector output
outDir = 'G:\HAT_A_06\HAT_A_06_d1-3\TPWS_new'; % directory where you want to save your TPWS file
    ...
    if dirSet(itr0).isdir &&~strcmp(dirSet(itr0).name,'.')&&...
        ...
        for itr2 = 1:lfs
            ...
            if ~isempty(clickTimes)&& size(specClickTf,2)>1
                ...
                keepers1 = find(ppSignal >= ppThresh);
                
                % % look for peak freq and identify those clicks with peaks below a certain value
                % Can easily flip this to be all clicks >= instead of <=
                peakFrIdx = max(specClickTf,[],2);
                peakFr = f(peakFrIx);
                keepers2 = find(peakFr<=15);
                % %
                
                keepers = intersect(keepers1,keepers2);
            ...
            if (size(clickTimesVec,1)>= 1000000 && (lfs-itr2>=10))|| itr2 == lfs
